...
import { getProjectById, createTask, updateProject } from "../../api";
...
const ProjectDetail = ({}) => {
  ...
  const fetchProjectDetails = async () => {
    ...
    getProjectById(id).then((data) => {
      ...
      setTasks(data.tasks.map((task) => task.data));
      setLoading(false);
    });
  };
  ...
  const handleTaskCreate = () => {
    createTask(id, task.name, task.description).then((data) => {
      console.log("data", data);
  ...
  const updateProjectDetails = async () => {
    updateProject(id, project.description).then((data) => {
